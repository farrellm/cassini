...
**A ninth round, 2026-09-23**, came from reviewing `DESIGN.md` against the corpus, and found two
claims that were faithful to their sources' words and still wrong about the thing described:

- **Ishii's arity parameter does not keep ℚ[x,y,z] and ℚ[w,x,y] apart.** Both rings have arity 3, so
  they share one `Polynomial ℚ 3` type. The separation the paper's introduction sets as a goal is
  delivered by a second layer, `LabPoly`, which carries the variable names as a type-level list (§2.3:
  "Our library also provides the LabPoly type, which converts existing polynomial types into
  'labelled' ones"). This file, `cas-haskell.md` and `../references/papers/haskell/CLAUDE.md` all
  credited the arity parameter; all three now name `LabPoly`.
- **numeric-prelude's `Eq`/`Show` superclass objection is historical.** The README still makes it,
  and the quote is accurate, but `Num` in today's `base` has no superclasses — checked with
  `:info Num` on GHC 9.12.4. It is now marked as such on the entry, in `cas-haskell.md` and in the
  directory annotation; the other three objections stand.
