...
| [`petkovsek_wilf_zeilberger1996_a_eq_b.pdf`](./papers/textbooks/petkovsek_wilf_zeilberger1996_a_eq_b.pdf) | ***A=B***<br>*Marko Petkovšek, Herbert S. Wilf, Doron Zeilberger (1996)*<br>**The held file is the authors' free electronic edition, not the A K Peters 1996 printing** — internal title `PwzForPC.DVI`, created 27 April 1997, and it contains no publisher, no ISBN, no copyright page and no "1996". Byte-identical (md5 `02eff372…`) to `www2.math.upenn.edu/~wilf/AeqB.pdf` as served 2026-09-01. Same text; different artifact from the citation. | UPenn / Rutgers Open Access | 217 | 1.19 MB |
| [`karr1981_summation_in_finite_terms.pdf`](./papers/textbooks/karr1981_summation_in_finite_terms.pdf) | **Summation in Finite Terms**<br>*Michael Karr (1981)*<br>Difference-field (ΠΣ) summation — the discrete analogue of Risch, and the half of symbolic summation *A=B* omits. <br>**Partial letter-spacing defect** — scattered prose extracts as "p a p e r", "c o n c e r n e d w i t h", so greps *under-count* rather than fail (`theorem`: 76 visible of 100 present). Indexed as "clean" until round 6; it is not. | *JACM* 28(2):305–350 / DOI: [`10.1145/322248.322255`](https://doi.org/10.1145/322248.322255) — user-supplied | 46 | 1.4 MB |
...
| File | Document & Authors | Source / Identifier | Pages | Size |
| :--- | :--- | :--- | :--- | :--- |
| [`ishii2018_purely_functional_cas_haskell.pdf`](./papers/haskell/ishii2018_purely_functional_cas_haskell.pdf) | **A Purely Functional Computer Algebra System Embedded in Haskell**<br>*Hiromi Ishii (2018)* | arXiv: [`1807.01456`](https://arxiv.org/abs/1807.01456) — **the pre-print**, not the CASC 2018 chapter; it carries only the book DOI `10.1007/978-3-319-99639-4`, and neither LNCS 11077 nor pp. 288–303 appear in it | 16 | 252.6 KB |
...
| [`algebra_hackage.html`](./papers/haskell/algebra_hackage.html) | The `algebra` Hackage page — "Constructive abstract algebra". The source for what Kmett's hierarchy actually contains: an additive/multiplicative split, `Monoidal`/`Group`, `Rig`/`Rng`/`Ring`, `Module`, `Algebra`/`Coalgebra`, and the `Numeric.Domain.*` tower. **It has no `Magma` and prescribes `RebindableSyntax` nowhere** — both were asserted in the notes until the eighth round. Fetched 2026-09-01. | [`hackage.haskell.org/package/algebra`](https://hackage.haskell.org/package/algebra) | — | 31.1 KB |
| [`constructive_algebra_hackage.html`](./papers/haskell/constructive_algebra_hackage.html) | The `constructive-algebra` Hackage page — the anchor for "Classical structures are implemented without Noetherian assumptions", and for the Bezout-instead-of-principal-ideal-domain design the notes describe. Fetched 2026-09-01. | [`hackage.haskell.org/package/constructive-algebra`](https://hackage.haskell.org/package/constructive-algebra) | — | 18.9 KB |
...
`olah_hasksymb_readme.html`, `poly_hackage.html`, `dumb_cas_hackage.html`, `sbv_docs_data_sbv.html`, `algebra_hackage.html` and `constructive_algebra_hackage.html` are for. Two further
