...
| `pickering2014_a_la_carte_closed_type_families.html` | Pickering's 2014 reimplementation of the à-la-carte machinery with closed type families — the "productionized descendant" the notes name alongside Bahr & Hvitved. Read it for how `(:<:)` looks once GHC has closed type families; the paper's version predates them. |
| `wadler2008_data_types_a_la_carte_blog.html` | Wadler's blog post of 28.2.08, and the **actual source** of "presents the best solution to the Expression Problem that I've seen in Haskell (well, Haskell with `-fglasgow-exts`)". That verdict is not in Swierstra's paper, which cites Wadler only as the problem's namer. |
